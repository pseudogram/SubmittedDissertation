...
class CMAES:
    ...
    def result(
            self ):  # return best params so far, along with historically best reward, curr reward, sigma
        r = self.es.result
...
class SimpleGA:
    ...
    def tell( self, reward_table_result ):
        ...
        assert (len(
            reward_table_result) == self.popsize), "Inconsistent reward_table size reported."
    ...
    def result(
            self ):  # return best params so far, along with historically best reward, curr reward, sigma
        return (
        self.best_param, self.best_reward, self.curr_best_reward, self.sigma)
...
class OpenES:
    ...
    def ask( self ):
        ...
        self.solutions = self.mu.reshape(1,
                                         self.num_params) + self.epsilon * self.sigma
    ...
    def tell( self, reward_table_result ):
        ...
        change_mu = 1. / (self.popsize * self.sigma) * np.dot(self.epsilon.T,
                                                              normalized_reward)
    ...
    def result(
            self ):  # return best params so far, along with historically best reward, curr reward, sigma
        return (
        self.best_mu, self.best_reward, self.curr_best_reward, self.sigma)
...
class PEPG:
    ...
    def tell( self, reward_table_result ):
        ...
        if (
                self.learning_rate_decay < 1 and self.learning_rate > self.learning_rate_limit):
            self.learning_rate *= self.learning_rate_decay
